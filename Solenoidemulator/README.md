...
Solenoidemulaterは、PCと接続されたキーボードのタイピングに合わせてソレノイド／タイプライターのような金属音を模倣した音色、バイブレーションによる感覚的なフィードバックを発生させるアプリケーションです。<br>
...
- **ボタン説明**
  ...
  - **画面タップ**
      - 長押し:SettingモードのOn
        - Cボタンのタップで描画UI設定画面との切り替え
        - ABC何れかの長押しでSolenoid画面へ復帰
...
- **Settingモード説明**
  ...
  - **Volume**
    - 音量を変更します
  - **描画設定切り替え(Cボタンで設定画面を表示)**
    - UI Mode:solenoid描画 or コクピットUI風描画を切り替え。
    - Invert:HUD向けミラーモードの切り替え。<br><br>
...
![image](https://github.com/user-attachments/assets/b25f604d-4083-415a-a349-036fad0e1a6c)
