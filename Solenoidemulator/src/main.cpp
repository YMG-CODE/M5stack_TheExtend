...
volatile bool sliderActive = false;
...
uint8_t soundVolume = 150;       // ソレノイド音量(🔴スピーカー保護のリミッターMAX80)
...
void loadConfig() {
  ...
  // 🔴 安全リミッタ
  soundVolume = constrain(soundVolume, 0, 150);
...
void handleConfigTouch() {
  ...
  if (t.isPressed()) {
    ...
    // Volume slider
      ...
      soundVolume = constrain(map(t.x, 20, 240, 0, 150), 0, 150);
  ...
  if (t.wasPressed() && t.y > 195 + offsetY && t.y < 220 + offsetY) {
...
void checkTouchToConfig() {
  ...
  // 長押し判定
  if (t.isPressed() && touchStart != 0 &&
      millis() - touchStart > 800)
  {
    configMode = true;
    settingsTogglePage = false;
    configEntryTime = millis();
    touchStart = 0;          // ★重要：リセット
    drawConfigUI();
  }
}
...
if (configMode)
{
  ...
  if (!configMode && M5.BtnC.wasPressed())
    cDownMs = millis();
  if (!configMode && M5.BtnC.wasReleased()) {
